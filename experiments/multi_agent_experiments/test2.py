...
def create_knowledge_base(documents):
    ...
    return create_retriever_tool(retriever, "research_paper_knowledge_base", "Can ")
...
def create_agents(tools, llm):
    ...
    research_agent = create_react_agent(llm, tools=[tools["retriever_tool"]])
...
def define_workflow(agents, llm):
    ...
    workflow.add_node("Researcher", lambda state: agent_node(state, agents["research_agent"], "Researcher", state["log"]))
...
def main():
    setup_environment()
    documents = load_and_prepare_documents("./records/neurips_workshop.pdf")
    ...
    for state in graph.stream(
        {
            "messages": [
                HumanMessage(
                    content="What is the model name which is being used in the paper to understand sketches? Can you print the paper title in Python?"
                )
            ],
            "log": log,
        }
